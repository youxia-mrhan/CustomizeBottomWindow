...
public class AllSortDownHo extends DownBaseView {
    ...
    private List<SortWindowBean.AllSortListDTO> allSortList;
    ...
    // 二级联动：全部分类，点击右侧选项
    private void updateAllSortRightState(int currentIndex, List<SortWindowBean.AllSortListDTO.AllSortChildDTO> detail) {
        // 右侧分类详情
        TextView btn;
    ...
    private void updateAllSortLeftState(int currentIndex) {
        ...
        // 右侧分类详情
        List<SortWindowBean.AllSortListDTO.AllSortChildDTO> detail = allSortList.get(currentIndex).getAllSortChild();
    ...
    // 悬浮框内容：全部分类
    public View getAllSortFilter(SortWindowBean sortWindowBean) {
        ...
        allSortList = sortWindowBean.getAllSortList();
        ...
        // 右侧分类详情，初始化
        List<SortWindowBean.AllSortListDTO.AllSortChildDTO> detail = allSortList.get(0).getAllSortChild();
        for (int i = 0; i < detail.size(); i++) {
            TextView detailText = new TextView(getContext());
...
//            Log.d("TAG", str.toString());
    ...
    // 选中右侧
    private void selectedRightItem(SortWindowBean.AllSortListDTO.AllSortChildDTO selected) {
        // 循环查找是否存在，如果存在不执行，不存在 清除原来加入的二级右侧选项，再加入二级右侧选项

        SortWindowBean.AllSortListDTO.AllSortChildDTO s = selected;

        boolean isExits = false;
        int previousId = 0;
        for (int i = 0; i < selectedItems.size(); i++) {
            if (selectedItems.get(i) instanceof SortWindowBean.AllSortListDTO.AllSortChildDTO) {
                previousId = i;
                SortWindowBean.AllSortListDTO.AllSortChildDTO s2 = (SortWindowBean.AllSortListDTO.AllSortChildDTO) selectedItems.get(i);
    ...
    // 选中左侧
    private void selectedLeftItem(SortWindowBean.AllSortListDTO selected) {
        // 循环查找是否存在，如果存在不执行，不存在 清除原来所有选项
        // 加入一级选项，并赋予二级选项 索引0的对象（要先判空）

        SortWindowBean.AllSortListDTO s = selected;
        boolean isExits = false;
        for (int i = 0; i < selectedItems.size(); i++) {
            if (selectedItems.get(i) instanceof SortWindowBean.AllSortListDTO) {
                SortWindowBean.AllSortListDTO s2 = (SortWindowBean.AllSortListDTO) selectedItems.get(i);
        ...
        SortWindowBean.AllSortListDTO.AllSortChildDTO rightSelected;
    ...
    private void initSelected() {
        ...
        SortWindowBean.AllSortListDTO leftSelected;
